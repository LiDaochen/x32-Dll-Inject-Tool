...
BOOL C注入工具Dlg::OnInitDialog()
{
	...
	m_List.InsertColumn(1, _T("加载基址"), LVCFMT_LEFT, 100);
	m_List.InsertColumn(3, _T("模块路径"), LVCFMT_LEFT, 370);
...
BOOL C注入工具Dlg::FreeDll(DWORD dwProcessId, CString szDllPathname)
{
	...
	if (hMouduleInjectDll)
	{
		...
		dwFreeAddr = (DWORD)GetProcAddress(hMoudule, "FreeLibrary");
		// 5: 创建远程线程，加载DLL
		size_t nNum = (DWORD)hMouduleInjectDll;
...
void C注入工具Dlg::OnBnClickedButton5()
{
	...
	do
	{
		...
		DWORD nNum = (DWORD)me32.hModule;  // 将HMOUDLE转换为DWORD
		TCHAR strMoudle[20];    // 将DWORD转换为Wchar*
		wsprintf(strMoudle, L"0x%x", nNum);
		m_List.SetItemText(nIndex, 1, strMoudle);
		// 5.4 将模块路径插入到item第2列
